...
Generation Server 用于部署大语言模型（LLM），并通过接收 Prompt Server 提供的输入完成响应生成。当前使用vLLM作为模型部署后端。
...
generate：接收prompt server提供的prompt 输入，调用支持 OpenAI API 协议的 LLM 接口进行生成，最终返回回答字符串列表。