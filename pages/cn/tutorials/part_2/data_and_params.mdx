...
在最简单的串行 pipeline 中，这种默认绑定没有问题。但如果你在循环或分支中多次调用 retriever_search，希望传入不同的数据（比如第一次叫 q_ls，第二次叫 sub_q_ls），那你就需要一个方法来告诉 pipeline：这些变量其实是“同义词”。
...
其中：
- `tool_param_name`：工具注册时定义的参数名，即函数签名中的形参名称。
- `actual_variable_name`：当前 Pipeline 中实际传入或接收的变量名，可自定义命名或引用已有变量。
...
说明：无论工具原本输出变量名叫什么（例如`ret_psg`），现在都将其绑定为 `round1_result`，用于 pipeline 后续步骤引用。例如：
