...
编辑生成的 `parameter/webnote_parameter.yaml` 来确保参数设置符合预期。例如：

```yaml parameter/webnote_parameter.yaml
prompt:
...
若不在 parameter.yaml 中为每个模板显式命名字段，而统一使用 `template:`，将可能导致 build 阶段参数冲突。`webnote_parameter.yaml` 的格式如下所示：

```yaml examples/webnote_parameter.yaml
benchmark:
  benchmark:
    key_map:
