---
title: "Custom Sever：添加无法归类的自定义 Tool"
icon: "square-terminal"
---

## Server 作用

`Custom Server` 是专门用于存放无法归类到标准模块（如 `Retriever`、`Generation`、`Evaluation` 等）中的自定义工具函数（`Tool`）的 Server。

所有不属于核心 Server 的逻辑工具（如数据清洗、关键词提取、特定规则逻辑等）都可以归入 `Custom Server`。
